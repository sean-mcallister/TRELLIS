    ...
    def preprocess_image(self, input: Image.Image) -> Image.Image:
        ...
        def resize_with_aspect(img: Image.Image, max_dim: int) -> Image.Image:
            ...
            if img.size[0] > img.size[1]:
                return img.resize(
                    (max_dim, int(max_dim / aspect)), Image.Resampling.LANCZOS
                )
            else:
                return img.resize(
                    (int(max_dim / aspect), max_dim), Image.Resampling.LANCZOS
                )

        # Force max dimension of 1024 before background removal
